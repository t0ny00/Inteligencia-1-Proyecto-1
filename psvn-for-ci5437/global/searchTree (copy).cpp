...
int main(int argc, char **argv) {
    ...
    int d, ruleid,node_counter,depth,tmp,hist,child_hist;
    ruleid_iterator_t iter;
    int resultsSumm[3][20] = {0}; 
    ...
    while( !open.Empty() && depth < 4) {
        ...
        while( (ruleid = next_ruleid(&iter) ) >= 0 ) {
            //if (fwd_rule_valid_for_history(hist,ruleid)==true) continue;
